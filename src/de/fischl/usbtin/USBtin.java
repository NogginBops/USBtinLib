...
/**
 * Represents an USBtin device. Provide access to an USBtin device
 * (http://www.fischl.de/usbtin) over virtual serial port (CDC).
 *
...
public class USBtin implements SerialPortEventListener {
    ...
    protected CopyOnWriteArrayList<CANMessageListener> listeners = new CopyOnWriteArrayList<CANMessageListener>();

    /** Transmit fifo */
    protected ConcurrentLinkedQueue<CANMessage> fifoTX = new ConcurrentLinkedQueue<CANMessage>();
    ...
    /**
     * Get firmware version string. During connect() the firmware version is
     * requested from USBtin.
     * 
    ...
    /**
     * Get hardware version string. During connect() the hardware version is
     * requested from USBtin.
     * 
    ...
    /**
     * Get serial number string. During connect() the serial number is requested
     * from USBtin.
     *
    ...
    /**
     * Connect to USBtin on given port. Opens the serial port, clears pending
     * characters and send close command to make sure that we are in
     * configuration mode.
     * 
     * @param portName
     *            Name of virtual serial port
     * @throws USBtinException
     *             Error while connecting to USBtin
     */
    ...
    /**
     * Open CAN channel. Set given baudrate and open the CAN channel in given
     * mode.
     * 
     * @param baudrate
     *            Baudrate in bits/second
     * @param mode
     *            CAN bus accessing mode
     * @throws USBtinException
     *             Error while opening CAN channel
     */
     ...
     * 
     * @throws USBtinException
     *             Error while closing CAN channel
     */
     ...
     *             Error while accessing USBtin
     * @throws SerialPortTimeoutException
     *             Timeout of serial port
     */
     ...
     * @return Response from USBtin
     * @throws SerialPortException
     *             Error while talking to USBtin
     * @throws SerialPortTimeoutException
     *             Timeout of serial port
     */
    public synchronized String transmit(String cmd) throws SerialPortException, SerialPortTimeoutException {
    ...
    /**
     * Handle serial port event. Read single byte and check for end of line
     * character. If end of line is reached, parse command and dispatch it.
     * 
     * @param event
     *            Serial port event
     */
                ...
                for (byte b : buffer) {
                    if ((b == '\r') && incomingMessage.length() > 0) {
                        ...
                        if (cmd == 't' || cmd == 'T' || cmd == 'r' || cmd == 'R') {
                            ...
                            // remove first message from transmit fifo and send
                            // next one
     ...
     * 
     * @throws USBtinException
     *             On serial port errors
     */
    protected synchronized void sendFirstTXFifoMessage() throws USBtinException {
        ...
        /*
         * We keep the message in the queue so that when sending new messages we
         * don't assume that we can send a message yet. This could lead to two
         * threads trying to write to the serial port at the same time.
         */
        CANMessage canmsg = fifoTX.peek();

        try {
            serialPort.writeBytes((canmsg.toString() + "\r").getBytes());

            // Now that the message has been sent, it's safe to remove the
            // message.
            fifoTX.poll();
        } catch (SerialPortException e) {
            throw new USBtinException(e);
     ...
     * 
     * @param filterid
     *            Filter identifier (0 = RXF0, ... 5 = RXF5)
     * @param registers
     *            Register values to write
     * @throws USBtinException
     *             On serial port errors
    ...
    /**
     * Set hardware filters. Call this function after connect() and before
     * openCANChannel()!
     * 
     * @param fc
     *            Filter chains (USBtin supports maximum 2 hardware filter
     *            chains)
     * @throws USBtinException
     *             On serial port errors
     */
    public void setFilter(FilterChain[] fc) throws USBtinException {
         ...
         * 
         * RXM0 RXM1 | | RXF0 RXF2 RXF1 RXF3 RXF4 RXF5
         */
        ...
        if (fc.length == 2) {
            ...
            if ((fc[0].getFilters().length > 2) || (fc[1].getFilters().length > 4)) {
                throw new USBtinException("Filter chain too long: " + fc[0].getFilters().length + "/"
                        + fc[1].getFilters().length + " (maximum is 2/4)!");
            }
